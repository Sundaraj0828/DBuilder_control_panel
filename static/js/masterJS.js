function draw(){  
    ...
    if(w_ft && h_ft){
        ...
        if(selectedPanel == 'Flush'){
            f_options = document.getElementById('flush_panels').value;
        }
    ...
    function get_panel_count(panel, door_w){
        var panel_c = 0
        if(panel == 'Short' || f_options == 'Short_p'){
        ...
        else if(panel == 'Long' || f_options == 'Long_p' || panel == 'Bead Board' || panel == 'Long Bead Board' || panel == 'Recessed' || panel == 'Raised'){
            if(door_w < 12){
                panel_c = 2
            }
    ...
    function get_color(base_c){
        ...
        if(selectedColor == '#181818'){
            ...
            darker2 = LightenDarkenColor(base_c,-7);
            darker3 = LightenDarkenColor(base_c,-9);
        }
        else{
    ...
    function get_panel_width(s_panel){
        if(s_panel== 'Short' || f_options == 'Short_p'){
            p_w = panel_width['short_panel_w']
        }
        else if(s_panel== 'Long' || f_options == 'Long_p' || s_panel == 'Bead Board' || s_panel == 'Long Bead Board' || s_panel == 'Recessed' || s_panel == 'Raised'){
...
function get_spacings(panel){
    ...
    if(w_ft == 8){
        ...
        if(panel == 'Short' || f_options == 'Short_p'){
            ...
            var c_s_bb = c_spacing_short[5];
        }
    }
    e_s_spacing = get_panel_size(w_ft, h_ft, e_s_spacing);
    c_spacing = get_panel_size(w_ft, h_ft, c_spacing);
    if(c_s_bb){
        c_spacing_bb = get_panel_size(w_ft, h_ft, c_s_bb);
    }
    if(c_spacing_bb){
        var spacings = [e_s_spacing, c_spacing, c_spacing_bb]
    }
    else{
        var spacings = [e_s_spacing, c_spacing]
    }
    ...
    // door divider ------------------------------
    function door_divider(h_ft, panel_r_count, height_comp, base_clr){
        var p_row = 0;
        var s_hght = 0;
        var r_hght = 0;
    ...
    function stamp_outline(x, y, w, h, c_shades){ 
        ctx.strokeStyle = c_shades[2];
        ctx.strokeRect(x+0.5, y+0.5, w, h);
        ctx.stroke();
        
        ctx.strokeStyle = c_shades[5];
    ...
    function top_shadow(x, y, w, c_shades, shadow_width){
        var sw = shadow_width;
        ctx.beginPath();
        ctx.fillStyle = c_shades;
        // ctx.fillStyle = 'green' ;
        ctx.strokeStyle = 'transparent';
        ctx.moveTo(x, y);
        ctx.lineTo(x+w, y);
    ...
    function right_shadow(x, y, h, c_shades, w, s_width){
        var sw = s_width;
        ctx.beginPath();
        ctx.fillStyle = c_shades;
    ...
    function bottom_shadow(x, y, w, h, c_shades, s_width){
        var sw = s_width;
        ctx.beginPath();
        ctx.fillStyle = c_shades;
        // ctx.fillStyle = 'blue';
    ...
    function divider_left(x, lx, y, h, c_shade){ 
        ctx.beginPath();
        ctx.fillStyle = c_shade[4];
        ctx.strokeStyle = 'transparent';
        ctx.moveTo(x, y);
        ctx.lineTo(x, y+h);
        ctx.lineTo(lx, y+h+3);
    ...
    function divider_right(x, lx, y, h, c_shade){
        ctx.beginPath();
        ctx.fillStyle = c_shade[1];
        ctx.strokeStyle = 'transparent';
        ctx.moveTo(x, y-3);
        ctx.lineTo(x, y+h+3);
        ctx.lineTo(lx, y+h);
    ...
    function beadboard_area(x, y, w, h, c_shades){
        console.log('bb area', w);
        var bb_count = 5;

        var x_pos = (+x) + (w/bb_count);

        for(p = 1; p<bb_count; p++){ 
            // if(p == bb_count){
            //     break;
            // } 
            console.log('--------------------',x_pos);
            divider_left(x_pos, x_pos + 2, y, h, c_shades);
            divider_right(x_pos +2 , x_pos + 4, y, h, c_shades);
            x_pos = x_pos + (w/bb_count) + 1;
        }
    }

    function beadboard_base(x, y, w, h, c_shades){
        if(selectedPanel == 'Raised'){
            var s_width = 8;
        }
        else{
            var s_width = 4;
        ...
        if(selectedPanel == 'Bead Board'){
            var bb_count = 5;
        }
        else if(selectedPanel == 'Long Bead Board'){
            var bb_count = 8;
        }
        ...
        left_shadow(x + 5, y, y + h, c_shades[1], w, s_width);
        top_shadow(x + 5, y, w, c_shades[0], s_width);
        right_shadow(x + w, y, h, c_shades[4], w, s_width);
        bottom_shadow(x + 4, y, w - 5, h, c_shades[5], s_width);
        ...
        for(t=1; t<bb_count; t++){
            ctx.beginPath();
            ctx.fillStyle = c_shades[4];
            ctx.moveTo(x_pos + (t*(w_val/bb_count)), y_pos-1);
            ctx.lineTo(x_pos + (t*(w_val/bb_count)), (y_pos) + h_val+1);
            ctx.lineTo(x_pos + (t*(w_val/bb_count) - 2), (y_pos) + h_val);
            ctx.lineTo(x_pos + (t*(w_val/bb_count) - 2), (y_pos));
            ctx.lineTo(x_pos + (t*(w_val/bb_count)), (y_pos - 1));
            ctx.fill();

            ctx.beginPath();
            ctx.fillStyle = c_shades[1];
            ctx.moveTo(x_pos + (t*(w_val/bb_count)), y_pos-1);
            ctx.lineTo(x_pos + (t*(w_val/bb_count) + 2), (y_pos));
            ctx.lineTo(x_pos + (t*(w_val/bb_count) + 2), (y_pos) + h_val);
            ctx.lineTo(x_pos + (t*(w_val/bb_count)), (y_pos) + h_val + 1);
    ...
    function stamp_shadow(x, y, w, h, c_shades){
        s_width = 4;
        left_shadow(x, y, y + h, c_shades[5], w, s_width);
        top_shadow(x, y, w, c_shades[4], s_width);
        right_shadow(x + w, y, h, c_shades[2], w, s_width);
        bottom_shadow(x, y, w, h, c_shades[1], s_width);

        if(selectedPanel != 'Recessed' || selectedPanel == 'Raised'){
            beadboard_base(x, y + 4, w - 4, h - 8, c_shades);
        }
    }

    function pair_of_stamps(x, y, w, h, c_stl_spacing, c_shades, bb_spacing){
        ...
        if(selectedPanel == 'Bead Board'){
            p_count = 2;
            p_w = (w/2) - (bb_spacing/2);
        }
        else if(selectedPanel == 'Recessed' || selectedPanel == 'Raised'){
            p_count = 4;      
            p_w = (w/4) - (bb_spacing/1.35);
        }

        var x_pos = x;
        for(i = 0; i<p_count; i++){
            stamp_shadow(x_pos+1, y+1, p_w-2, h-2, c_shades);
            stamp_outline(x_pos, y, p_w, h, c_shades);

            x_pos = x_pos + p_w + bb_spacing ;
        } 
    ...
    function stamps_col_bb(x, y, p_count, r_height, c_stl_spacing, w, h, beveled_edge, bb_spacing){
        ...
        if(selectedPanel == 'Bead Board' || selectedPanel == 'Recessed' || selectedPanel == 'Raised'){
            pair_of_stamps(x, y, w, h, c_stl_spacing, color_shades, bb_spacing);             
        }
        else if(selectedPanel == 'Long Bead Board'){
            stamp_shadow(x+2, y+2, w-4, h-4, color_shades);

            stamp_outline(x, y, w, h, color_shades);
        }
        ...
        for(i=0; i<panel_r_count; i++){
            ...
            if(row_count != w_row){
                for(p = 0; p<p_count; p++){
                    ...
                    if(selectedPanel == 'Bead Board' || selectedPanel == 'Long Bead Board' || selectedPanel == 'Recessed' || selectedPanel == 'Raised'){
                        stamps_col_bb(panel_x, sc_height, p, r_hght, c_spacing, p_wdth, p_hght, b_edge, c_spacing_bb);    
                    }
                    else{
    ...
    function door_area(d_width, d_height, b_color, selectedPanel){
        ...
        if(d_width && d_height){
            ...
            if(selectedPanel){
                if(selectedPanel=='Short' || selectedPanel== 'Long' || f_options == 'Short_p' || f_options == 'Long_p' || selectedPanel == 'Bead Board' || selectedPanel == 'Long Bead Board' || selectedPanel == 'Recessed' || selectedPanel == 'Raised'){
            ...
            if(selectedPanel ){
                if(selectedPanel == 'Long' || selectedPanel == 'Short' || f_options == 'Short_p' || f_options == 'Long_p' || selectedPanel == 'Bead Board' || selectedPanel == 'Long Bead Board' || selectedPanel == 'Recessed' || selectedPanel == 'Raised'){
            ...
            if(selectedGlassType != 'closed'){
                if(selectedPanel == 'Short' || f_options == 'Short_p'){
                    if(selectedWindow){
                        if(selectedWindow == 'sp_wagonWheel'){
                            if(w_ft == 10){
                                alert('Short panel WagonWheel is not supported with ' + w_ft + 'ft wide door');
                            ...
                            if(w_ft == 10 || w_ft == 12 || w_ft == 14 || w_ft == 15 || w_ft == 20){
                                alert('Short panel Sunray is not supported with ' + w_ft + 'ft wide door');
                                window_call(panel_r_count, panel_count, panel_wdth, panel_spacing, panel_hght, b_edge, 'sp_clear');                        
                            }
                            else{
                                window_call(panel_r_count, panel_count, panel_wdth, panel_spacing, panel_hght, b_edge, selectedWindow);                        
                ...
                else if(selectedPanel == 'Long' || f_options == 'Long_p' || selectedPanel == 'Bead Board' || selectedPanel == 'Long Bead Board' || selectedPanel == 'Recessed' || selectedPanel == 'Raised'){
                    console.log('heritage - windows');
                    if(selectedWindow){
                        if(selectedWindow == 'lpml'){
                            if(w_ft == 12 || w_ft == 14 || w_ft == 20){
                                alert('Long panel Moonlite is not supported with ' + w_ft + 'ft wide door');
                                window_call(panel_r_count, panel_count, panel_wdth, panel_spacing, panel_hght, b_edge, 'lp_clear');
                            }
                            else{
                                window_call(panel_r_count, panel_count, panel_wdth, panel_spacing, panel_hght, b_edge, selectedWindow);
                            }
                            ...
                            if(w_ft == 12 || w_ft == 14 || w_ft == 20){
                                alert('Long panel Arched Themes is not supported with ' + w_ft + 'ft wide door');
                                window_call(panel_r_count, panel_count, panel_wdth, panel_spacing, panel_hght, b_edge, 'lp_clear');
                            }
                            else{
                                window_call(panel_r_count, panel_count, panel_wdth, panel_spacing, panel_hght, b_edge, selectedWindow);
                            ...
                            if(w_ft == 12 || w_ft == 14 || w_ft == 20){
                                alert('Long panel Sunray is not supported with ' + w_ft + 'ft wide door');
                                window_call(panel_r_count, panel_count, panel_wdth, panel_spacing, panel_hght, b_edge, 'lp_clear');
                            }
                            else{
                                window_call(panel_r_count, panel_count, panel_wdth, panel_spacing, panel_hght, b_edge, selectedWindow);
            ...
            door_divider(h_ft, panel_r_count, h_comp, b_color);
        }  
        else{
    ...
    function rad_grad(x, y, w, h, count){
        // if(selectedPanel == 'Short' || selectedPanel == 'Long' || f_options == 'Short_p' || f_options == 'Long_p'){
            if(selectedGlassType == 'clear'|| selectedGlassType == 'frost'){      
                win_grad = ctx.createLinearGradient(x, y, x+(w/2), y+50);
            }
            else if(selectedGlassType == 'dark_tint' ){
                win_grad = ctx.createLinearGradient(x, y, x+(w/2), y+(0.29*w));
            }
            // else{
            ...
            function decraTrim_SPStckn(x, y, w, h, b){
                ...
                for(k=y; k<=(y+h); k+=(h*0.4)){
                    if(k==y){
                        ...
                        if(selectedWindow == 'sp_stockton'){
                            ...
                            ctx.lineWidth = (0.21*b);
                            ctx.moveTo(x, k+(0.084*b));
                            ctx.lineTo(x+w, k+(0.084*b));
                            ctx.stroke();
                        }
                            ...
                            ctx.lineCap = 'round';
                            ctx.strokeStyle = base_color;
                            ctx.moveTo(x, k-(0.042*b));
                            ctx.lineTo(x+w, k-(0.042*b));
                            ctx.stroke();
                ...
                if(selectedWindow == 'stktn'){
                    ...
                    for(p=1; p<6; p++){
                        ...
                        ctx.lineCap = 'round';
                        ctx.moveTo(x+dt_x, y);
                        ctx.lineTo(x+dt_x, y+h-(0.084*b));
                        ctx.stroke();

                        // shadow ------
                        ctx.beginPath();
                        ctx.strokeStyle = 'rgba(0,0,0,0.1)';
                        ctx.lineWidth = (0.13*b);
                        ctx.moveTo(x+dt_x+(0.084*b), y);
                        ctx.lineTo(x+dt_x+(0.084*b), y+h-(0.21*b));
                        ctx.stroke();
                    ...
                    for(p=1; p<4; p++){
                        ...
                        ctx.lineWidth = (0.21*b);
                        ctx.moveTo(x+dt_x+(0.17*b), y);
                        ctx.lineTo(x+dt_x+(0.17*b), y+h-(0.084*b));
                        ctx.stroke();
                    ...
                    for(m = 0; m<(x+w); m+=x+(3.05*b)){
                        ...
                        ctx.lineCap = 'round';
                        ctx.moveTo(m, y);
                        ctx.lineTo(m, y+h-(0.084*b));
                        ctx.stroke();
    
                        ctx.beginPath();
                        ctx.strokeStyle = 'rgba(0,0,0,0.1)';
                        ctx.lineWidth = (0.21*b);
                        ctx.moveTo(m+(0.084*b), y);
                        ctx.lineTo(m+(0.084*b), y+h-(0.084*b));
                        ctx.stroke();
                    }
                }
            }
        ...
        function windows_cascade(margin_x, margin_y, window_w, window_h,p_cnt, b_edge, count){
            ...
            function glass_frame_lpww(x, y, w, h, b, c){
                ...
                if(selectedWindow == 'lpww'){
                    ...
                    for(d=1; d<=2; d++){
                        if(d==1){
                            frame_triangles_lpww(x+(0.59*b), y+(0.85*b), x+(4.66*b), x+(1.87*b), h, d, b);
                        }
                        else{
                            frame_triangles_lpww(x+w-(0.76*b), y+(0.85*b), x+w-(4.66*b), x+w-(2.04*b), h, d, b);
                        }            
                    }
                }
                else if(selectedWindow == 'sp_wagonWheel'){
                    ...
                    if(c%2){
                        curve_shadows_sp(x+w-(0.59*b), y+h-(0.64*b), x+w-(2.54*b), y+(0.64*b), x+(0.59*b), lighter2, c);
                        frame_triangles_sunray(x+w-(0.72*b), y+(0.72*b), x+(2.75*b), x+w-(2.54*b), y+(0.42*b), w, h, c, b);
                    }
                    else{
                        curve_shadows_sp(x+(0.59*b), y+h-(0.64*b), x+(2.50*b), y+(0.64*b), x+w-(0.59*b), lighter2, c);
                        frame_triangles_sunray(x+(0.72*b), y+(0.72*b), x+w-(2.33*b), x+(2.54*b), y+(0.42*b), w, h, c, b);
                    }
                }
                else if(selectedWindow == 'sp_sunray'){                    
                    if(c==0 || c==4){
            ...
            function frame_triangles_sunray(x1, y1, x3, q_x1, q_y1, w, h, c, b){
                ctx.beginPath();
                ctx.lineWidth = 1;
                ctx.strokeStyle = 'transparent';
                ctx.fillStyle = base_color;
                ...
                ctx.clip();
                    ...
                    if(c%2  == 0){
                        ...
                        ctx.quadraticCurveTo(q_x1, q_y1, x1, y1+h-(2.12*b));
                        ctx.lineTo(x1+(0.13*b), y1+h-(2.54*b));
                        ctx.quadraticCurveTo(x1+(2.33*b), y1-(0.42*b), x3-(1.27*b), y1+(0.13*b));
                    }
                    else{
                        ctx.fillStyle = lighter2;
                        ctx.moveTo(x3, y1);
            ...
            function frame_triangles_lpww(x, y, lx, qx, h, d, b){
                ...
                ctx.lineTo(lx, y);
                ctx.quadraticCurveTo(qx, y+(0.51*b), x+(0.084*b), y+h-(2.00*b));
                ...
                ctx.clip();
                    ...
                    ctx.moveTo(x+(0.084*b), y);
                    ctx.lineTo(x+(0.084*b), y+h-(2.00*b));
                    ctx.stroke();
