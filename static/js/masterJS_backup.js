function draw(){  
    ...
    var short_panel_w = 21.000
    var long_panel_w = 41.000

    // ------------------------------getting Width & height as ft
    ...
    // --------------------------------convert to px
    // function px_convertion(sec_h, rl_h, panel_r_count){
    //     console.log('sec_h---', sec_h.length)
    //     var panel_width = get_panel_size(w_ft, h_ft, p_w)
    //     var panel_height = get_panel_size(w_ft, h_ft, panel_h)
    //     var beveled_edge = get_panel_size(w_ft, h_ft, beveled_e)

        
    //     var section_height_18 = get_panel_size(w_ft, h_ft, sec_h)
    //     var section_height_21 = get_panel_size(w_ft, h_ft, sec_h)
    //     var rail_height_18 = get_panel_size(w_ft, h_ft, rl_h)
    //     var rail_height_21 = get_panel_size(w_ft, h_ft, rl_h)
    ...
    // -------------------------------------Height calculation
    function Height_maipulation(d_height){
        var height_components = {}

        
        if(p_w){
            p_width = get_panel_size(w_ft, d_height, p_w)
            height_components['panel_width'] = p_width
        }
        p_height = get_panel_size(w_ft, d_height, panel_h)
        height_components['panel_height'] = p_height
        b_edge = get_panel_size(w_ft, h_ft, beveled_e)        
        height_components['beveled_edge'] = b_edge
        if(e_s_spacing){
            end_stl_spacing = get_panel_size(w_ft, h_ft, e_s_spacing)       
            height_components['end_stile_spacing'] = end_stl_spacing
        }
        if(c_spacing){
            c_stl_spacing = get_panel_size(w_ft, h_ft, c_spacing)       
            height_components['center_spacing'] = c_stl_spacing
        }

        height_components['panel_count'] = panel_count

        // -------------------section Height and rail height
        if(d_height == 6 || d_height == 7.6 || d_height == 9){
            sec_ht = get_panel_size(w_ft, h_ft, section_h_18)
            rail_ht = get_panel_size(w_ft, h_ft, rail_h_18)
            ...
            sec_ht = get_panel_size(w_ft, h_ft, section_h_21)
            rail_ht = get_panel_size(w_ft, h_ft, rail_h_21)
            height_components['section_height'] = [sec_ht]
            height_components['rail_height'] = [rail_ht]
        }
            ...
            sec_ht_18 = get_panel_size(w_ft, h_ft, section_h_18)
            sec_ht_21 = get_panel_size(w_ft, h_ft, section_h_21)
            rail_ht_18 = get_panel_size(w_ft, h_ft, rail_h_18)
            rail_ht_21 = get_panel_size(w_ft, h_ft, rail_h_21)
        ...
        if(d_height == 6 || d_height == 6.3 || d_height == 6.6 || d_height == 6.9 || d_height == 7){
            height_components['panel_row_count'] = 4
        }
        else if(d_height == 7.6 || d_height == 7.9 || d_height == 8 || d_height == 8.3 || d_height == 8.6 || d_height == 8.9){
            height_components['panel_row_count'] = 5
        }
        else if(d_height == 9 || h_ft == 9.3 || d_height == 9.6 || d_height == 9.9 || d_height == 10 || d_height == 10.3 || d_height == 10.6){
            height_components['panel_row_count'] = 6
        }
        else if(d_height == 10.9 || d_height == 11 || d_height==11.3 || d_height == 11.6 || d_height == 11.9 || d_height == 12){
            height_components['panel_row_count'] = 7
        }

        // converted_details = px_convertion(height_components['section_height'], height_components['rail_height'], height_components['panel_row_count'])
    ...
    function configure_sections(door_h, sec_hght,rail_height, count, p_count){
        ...
        if(door_h == 6.3 || door_h == 7.9 || door_h == 9.3 || door_h == 10.9){
            ...
            if(count==0 || count == p_count || count == (p_count-2)){
                s_hght = sec_hght[1];
                r_hght = rail_height[1];
            }
            else{
    ...
    function get_section_height(door_h, section_h, panel_r_count, rail_h){
        var s_hght = 0;
        var r_hght = 0;
        console.log('section h length--------------', section_h.length)
        for(i = 0; i<panel_r_count; i++){
            if(section_h.length == 1){
                s_hght = (+s_hght) + (+sec_height);
                r_hght = rail_h
            }
            else if(section_h.length == 2){
                sec_h = configure_sections(door_h, sec_height,rail_h, i, panel_r_count)
                s_hght = (+s_hght) + (+sec_h[0]);

                r_h = configure_sections(door_h, sec_height,rail_h, i, panel_r_count)
                r_hght = r_h[1]
            }

            set_section_height(s_hght, i, panel_r_count); 
            stamp_area(door_h, s_hght, r_hght, panel_r); 
            s_hght+=1;
        }
    }

    function set_section_height(s_height, panel_row, p_r_count){
        if(panel_row < p_r_count-1){
            hr_divider(0, s_height, w, '#000');
    ...
    function door_divider(door_height){   
        height_comp = Height_maipulation(door_height);
        console.log('----------------- Height Componnents')
        console.log(height_comp)
        
        
        sec_height = height_comp['section_height']
        rl_height = height_comp['rail_height']
        panel_r = height_comp['panel_row_count']

            
        console.log('------------------divider') 
        console.log(rl_height)           
        get_section_height(door_height, sec_height, panel_r, rl_height);
        // stamp_area(door_height, sec_height, rl_height, panel_r);
    ...
    function configure_rail_h(door_h, rail_height, count, panel_r){
        if(door_h == 6.3 || door_h == 7.9 || door_h == 9.3 || door_h == 10.9){
            if(count==(panel_r)){
                r_hght = rail_height[1];
            }
            ...
            if(count==0 || count == panel_r || count == (panel_r-2)){
                r_hght = rail_height[1];
            }
            else{
                r_hght = rail_height[0]; 
    ...
    function stamp_area(door_height, s_height, rail_height, panel_r){
        var r_height = 0;
        
        console.log('Rail heihgt-------------',rail_height)      
        height_comp = Height_maipulation(door_height);
        panel_w = height_comp['panel_width']
        panel_h = height_comp['panel_height']
        end_stl_spacing = height_comp['end_stile_spacing']
        c_spacing = height_comp['center_spacing']
        panel_c = height_comp['panel_count']
        b_edge = height_comp['beveled_edge']

        console.log('------------------------------ stamp area');
        console.log('panel_count-------------', panel_c)
        for(p = 0; p<panel_r; p++){
            if(rail_height.length == 1){
                r_height = rail_height[0]
            }
            else if(rail_height.length == 2){
                r_h = get_railHeight(door_height, rail_height, panel_r, p);
                r_height = r_h
            }
            console.log('------stamp builder')
            stamps_col(end_stl_spacing, s_height, p, r_height, c_spacing, panel_w, panel_h, b_edge);
            // stamp_create();
        } 
    ...
    function stamps_col(x, panel_y, p_count, r_height, c_stl_spacing, panel_width, panel_height, beveled_edge){
        console.log('-------------stamps col------', r_height)
        ctx.beginPath();
        ctx.lineWidth = 1;
        ctx.fillStyle = base_color;
        ...
        left_outer_shadow(x+((panel_width+c_stl_spacing)*p_count), panel_y+r_height, beveled_edge, panel_height);
        top_outer_shadow(x+((panel_width+c_stl_spacing)*p_count), panel_y+r_height, beveled_edge, panel_width);
        right_outer_shadow(x+((panel_width+c_stl_spacing)*p_count), panel_y+r_height, beveled_edge, panel_height, panel_width);
        bottom_outer_shadow(x+((panel_width+c_stl_spacing)*p_count), panel_y+r_height, beveled_edge, panel_width, panel_height);
        
        left_inner_shadow(x+((panel_width+c_stl_spacing)*p_count)+(beveled_edge/2), panel_y+r_height, beveled_edge, panel_height);
        top_inner_shadow(x+((panel_width+c_stl_spacing)*p_count), panel_y+r_height, beveled_edge, panel_width);
        right_inner_shadow(x+((panel_width+c_stl_spacing)*p_count), panel_y+r_height, beveled_edge, panel_width, panel_height);
        bottom_inner_shadow(x+((panel_width+c_stl_spacing)*p_count), panel_y+r_height, beveled_edge, panel_width, panel_height);
    }
    ...
    // -----------------------------------------------outer shadow
    function left_outer_shadow(stamp_x, stamp_y, beveled_edge, panel_height){
        ctx.beginPath();
        ctx.fillStyle = '#372415';
